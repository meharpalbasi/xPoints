...
GW4 deadline is Saturday 12 September 2026 12:30 UTC; FPL usually marks the gameweek `data_checked` on the Monday or Tuesday. The hourly workflow grades it on its own (`score.py`, then `score_site.py`); nothing needs running by hand. Then, in order:

1. **Confirm the grade exists and the archives pre-date the deadline.** `scores/gw4.json` (`prediction.model_version` should be absent there, but `predictors_available` must list `model`), `predictions/gw4.json` and `predictions/gw4_model.json` with `generated_at` before 12:30 UTC on 12 September, and `gw4_model.json` rows carrying `model_version: xpoints-two-stage-blend-v3` and `p_start60`. If the archives were rewritten after the deadline, the run's `archive_allowed` guard failed: that is a bug to fix before anything else, and the row is not a fair grade.
2. **Read v3's first paired row** in `scores/gw4.json` (`metrics.all.model.mae` against `ep_next` and `zero`; `metrics.starters.model.spearman` with its interval against `ep_next`; `precision_at_20`; `captain_regret`). Compare with GW3's v2 row in `scores/gw3.json`. One gameweek is one sample; write the numbers into "Where things stand" and move on. Do not touch `MODEL_VERSION`, `BLEND_WEIGHT` or the feature set on the strength of it.
3. **Read the two P(60+) Brier scores on the same players.** The model's is `metrics.all.model_p60.brier` in `scores/gw4.json` (availability applied, beside `base_rate_brier`); the site's is `brier_60` beside `brier_base_rate` in `scores/site/gw4.json`. Both populations are every player with a row, so they are comparable directly. If the site's expected-minutes model (`app/lib/xmins.mjs` in fpl-app: recency-weighted starts, availability multiplier) beats the model's classifier by more than about 0.005, the cheapest next model change is to add the site's inputs that ARE in the history (recency-weighted starts over six gameweeks, minutes share) to `backtest.market_features` and rerun `python backtest.py --out backtests/2025-26_v4.json`; the ones that are not (status, news recency, chance) still cannot be backtested and stay a post-model rule.
4. **Check the gate moved.** `scorecard.json` `summary.promotion.gameweeks_in_window` should read 2 with gameweeks `[3, 4]`, and the accuracy page card should say so. The rule needs six paired gameweeks, so the earliest verdict is after GW9 (mid-October). Do not shorten the window.
5. **Check the site's horizon grades landed.** `site_scorecard.json` should carry its first horizon rows (the GW4 projection made one deadline ahead is graded now; the ones made two to five deadlines ahead grade as those gameweeks finish) and `/app/accuracy` its two site sections. If `scores/site/gw4.json` is missing, run `python score_site.py` locally and read the warning it prints.
6. **fpl_dbt after its first runs with DBT-01** (fpl_dbt #26): the Railway log for the first pipeline run should show the "snapshot identity" line and "Step 5" passing; `raw.ingestion_runs` should hold a `complete` row per player-details run. A stopped pipeline means an incomplete ingest or a failed gate, which is the design working; read the log before changing anything.
